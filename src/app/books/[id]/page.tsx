...
// 【修复点 2】：把 params 的类型定义为一个 Promise
export default function BookDetailPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  // 【修复点 3】：使用 use() 解包 params，提取出真实的 id
  const { id } = use(params);
  ...
  const fetchBookData = async () => {
    // 【修复点 4】：把原来的 params.id 换成解包后的 id
