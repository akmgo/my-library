...
// ⚠️ 注意这里：我们引入了 getPresignedUrl 替代旧的上传方法
...
export default function BulkImportPage() {
  ...
  const handleImport = async () => {
    ...
    setIsImporting(true);
    addLog(`🚀 开始采用企业级直传架构批量导入，共计 ${NOTION_BOOKS.length} 本书...`);
    ...
    for (const book of NOTION_BOOKS) {
      try {
        ...
        try {
          ...
          addLog(`  -> ❌ 封面直传失败: ${uploadErr.message}，跳过入库。\n`);
          continue;
        }
    ...
    addLog("🎉 批量直传处理流程完毕！你可以去首页查看那堵完美的书墙了！");
    setIsImporting(false);
  };

  return (
    <div className="min-h-screen bg-slate-950 text-slate-300 p-10 max-w-4xl mx-auto">
      <h1 className="text-3xl font-bold text-white mb-6">📦 Notion 批量导入控制台 (直传版)</h1>
