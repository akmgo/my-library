...
const STATUS_OPTIONS = [{ id: "UNREAD", label: "待读" }, { id: "READING", label: "在读" }, { id: "FINISHED", label: "已读完" }];
const RATING_TEXTS = ["", "⭐ 一星毒草", "⭐⭐ 二星平庸", "⭐⭐⭐ 三星粮草", "⭐⭐⭐⭐ 四星推荐", "🔥 改变人生"];
const PREDEFINED_TAGS = ["哲学", "历史", "人文", "经典", "社会", "政治", "经济", "法律", "心理", "思考", "成长", "教育", "管理", "商业", "投资", "技术", "文学", "传记", "艺术", "宗教", "科普", "编程"];
        ...
        <div className="w-full sm:w-[240px] md:w-[280px] lg:w-[320px] shrink-0 mx-auto md:mx-0 relative z-20">
          <TiltCard>
            <div className="w-full aspect-[2/3] rounded-[2rem] overflow-hidden shadow-2xl dark:shadow-[0_20px_50px_rgba(0,0,0,0.5)] border border-white/50 dark:border-white/10 relative bg-slate-100 dark:bg-slate-900">
              <Image src={mainCoverUrl} alt={book.title} fill priority sizes="(max-width: 768px) 100vw, 33vw" className="object-cover" unoptimized={true} />
              ...
              {STATUS_OPTIONS.map((opt) => (
                <button
                  ...
                  className={`relative flex-1 flex items-center justify-center text-sm font-medium transition-colors duration-300 z-10 ${
                    book.status === opt.id ? "text-white" : "text-slate-500 dark:text-slate-500 hover:text-slate-800 dark:hover:text-slate-300"
                  }`}
                >
                  {book.status === opt.id && (
                    <motion.div layoutId="status-indicator" className="absolute inset-0 bg-indigo-500 dark:bg-indigo-600 rounded-lg shadow-md dark:shadow-lg" transition={{ type: "spring", stiffness: 300, damping: 25 }} />
                  )}
          ...
          <div className="flex flex-col space-y-3 mb-8 w-full">
            ...
            {book.status === "UNREAD" ? (
              <div className="h-11 w-full bg-slate-100/50 dark:bg-slate-950/50 border border-slate-200 dark:border-slate-700/50 rounded-xl overflow-hidden relative flex items-center px-4 shadow-inner">
                 ...
                 <span className="text-slate-400 dark:text-slate-500 text-sm font-medium tracking-wider relative z-10 italic">Waiting for the journey to begin...</span>
              </div>
            ) : (
              <div className="flex items-center gap-4 w-full">
                <div className="flex-1 relative">
                  <input type="date" value={book.startTime || ""} onChange={(e) => handleBookUpdate({ startTime: e.target.value })} className="bg-white/50 dark:bg-slate-950/50 border border-slate-200 dark:border-slate-700/50 text-slate-800 dark:text-white rounded-xl pl-10 pr-4 h-11 w-full outline-none focus:border-indigo-400 dark:focus:border-indigo-500 cursor-pointer transition-colors dark:[&::-webkit-calendar-picker-indicator]:filter dark:[&::-webkit-calendar-picker-indicator]:invert [&::-webkit-calendar-picker-indicator]:opacity-60 [&::-webkit-calendar-picker-indicator]:hover:opacity-100" />
                  <div className="absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none text-slate-400"><Calendar className="w-4 h-4" /></div>
                </div>
                <span className="text-slate-400 font-bold shrink-0">至</span>
                <div className="flex-1 relative">
                  <input type="date" value={book.endTime || ""} onChange={(e) => handleBookUpdate({ endTime: e.target.value })} disabled={book.status !== "FINISHED"} className={`bg-white/50 dark:bg-slate-950/50 border border-slate-200 dark:border-slate-700/50 text-slate-800 dark:text-white rounded-xl pl-10 pr-4 h-11 w-full outline-none focus:border-indigo-400 dark:focus:border-indigo-500 cursor-pointer transition-colors dark:[&::-webkit-calendar-picker-indicator]:filter dark:[&::-webkit-calendar-picker-indicator]:invert [&::-webkit-calendar-picker-indicator]:opacity-60 [&::-webkit-calendar-picker-indicator]:hover:opacity-100 ${book.status !== "FINISHED" ? "opacity-50 cursor-not-allowed" : ""}`} />
                  <div className="absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none text-slate-400"><Clock className="w-4 h-4" /></div>
                </div>
              </div>
            )}
          </div>
                ...
                {[1, 2, 3, 4, 5].map((star) => (
                  <Star
                    ...
                    className={`w-8 h-8 md:w-10 md:h-10 cursor-pointer transition-all duration-300 ${
                      (hoverRating || book.rating) >= star ? "text-yellow-500 fill-yellow-500 dark:text-yellow-400 dark:fill-yellow-400 drop-shadow-[0_0_10px_rgba(234,179,8,0.4)] dark:drop-shadow-[0_0_15px_rgba(250,204,21,0.5)] scale-110" : "text-slate-300 dark:text-slate-700 hover:text-slate-400 dark:hover:text-slate-500"
                    }`}
                  />
                ))}
              </div>
              ...
              <div className="ml-auto relative z-10 flex items-center justify-end gap-1 pr-4 w-32 hidden md:flex">
                {[1, 2, 3, 4, 5].map((starIndex) =>
                  starIndex <= (hoverRating || book.rating || 0) && (
                    <Sparkles key={`sparkle-${starIndex}`} className="w-5 h-5 text-yellow-500/40 dark:text-yellow-500/30 group-hover:text-yellow-500/60 dark:group-hover:text-yellow-500/50 animate-in fade-in zoom-in spin-in-12 duration-500" style={{ animationDelay: `${starIndex * 50}ms` }} />
                  )
                )}
              </div>
            </div>
